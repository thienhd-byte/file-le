...
async function executeBroadcast(msg, photoFileIds) {
    ...
    const sendOption = { 
        parse_mode: 'HTML',
        reply_markup: {
            inline_keyboard: [[{ text: "Xác nhận", callback_data: `confirm_${broadcastId}` }]]
        }
    };
    ...
    for (let i = 0; i < teacherCodes.length; i++) {
        ...
        try {
            ...
            if (isAlbum) {
                ...
                sentMessage = await bot.sendMessage(userProfile.teleChatId, messageHtml || 'Vui lòng xác nhận.', sendOption);
            } else if (isSinglePhoto) {
                sentMessage = await bot.sendPhoto(userProfile.teleChatId, photoFileIds[0], { ...sendOption, caption: messageHtml });
            } else if (msg.document) {
                sentMessage = await bot.sendDocument(userProfile.teleChatId, msg.document.file_id, { ...sendOption, caption: messageHtml });
            } else if (msg.video) {
                sentMessage = await bot.sendVideo(userProfile.teleChatId, msg.video.file_id, { ...sendOption, caption: messageHtml });
            } else {
                sentMessage = await bot.sendMessage(userProfile.teleChatId, messageHtml, sendOption);
            }
            // Lưu messageId
    ...
    try {
        ...
        await fileLogBroadcastHistory.set(
            `${ts} || ${senderId} || ${groupCode} || ${messageHtml.replace(/\|\|/g, '|')} || ${mediaType} || ${result.success.length} || ${result.failed.length} || ${broadcastId} || ${chainId} || ${parentBroadcastId} || ${broadcastTitle.replace(/\|\|/g, '|')} || ${messageType}`
        );
    } catch (historyError) {
    ...
    bot.on("message", async (msg) => {
        ...
        if (queueChatPrivate[msg.chat.id] && queueChatPrivate[msg.chat.id].startsWith('waiting_reminder_')) {
            ...
            const reminderOption = {
                parse_mode: 'HTML',
                reply_markup: {
                    inline_keyboard: [[{ text: "Xác nhận", callback_data: `confirm_${reminderBroadcastId}` }]]
                }
            };
            ...
            try {
                await fileLogBroadcastHistory.set(
                    `${ts} || ${msg.from.id} || ${selectedHistory.groupCode || ''} || ${reminderTitle.replace(/\|\|/g, '|')} || ${reminderType} || ${reminderSent} || ${reminderFailed} || ${reminderBroadcastId} || ${reminderChainId} || ${parentBroadcastId} || ${reminderTitle.replace(/\|\|/g, '|')} || ${messageType}`
                );
            } catch (error) {
