...
const allowedBroadcastSenderIds = new Set(['1230651720', 1230651720]);
...
async function showBroadcastList(bot, chatId, page = 0) {
    ...
    if (page > 0) {
        paginationButtons.push({ text: 'Trước', callback_data: `broadcast_page_${page - 1}` });
    }
    if (page < totalPages - 1) {
        paginationButtons.push({ text: 'Sau', callback_data: `broadcast_page_${page + 1}` });
    }
...
async function executeBroadcast(msg, photoFileIds) {
    ...
    for (let i = 0; i < teacherCodes.length; i++) {
        ...
        let userProfile = dataInit.userByUserName[teacherCode] || teacherGroup;
