...
const allowedBroadcastSenderIds = new Set(['151333233', 151333233, 1230651720, "1230651720", "1912632528", 1912632528, "2033518516", 2033518516, 2115970985]);
...
function renderTelegramEntitiesToHtml(text = '', entities = []) {
    ...
    for (const entity of entities) {
        ...
        openAt[start].push(item);
        closeAt[end].push(item);
    }
    ...
    for (let i = 0; i < safeText.length; i++) {
        if (closeAt[i] && closeAt[i].length) out += closeAt[i].map(t => t.closeTag).join('');
        if (openAt[i] && openAt[i].length) out += openAt[i].map(t => t.openTag).join('');
        out += escapeHtml(safeText[i]);
    }
    if (closeAt[safeText.length] && closeAt[safeText.length].length) out += closeAt[safeText.length].map(t => t.closeTag).join('');
    return out;
}
...
function parseBroadcastCommand(text = '') {
    ...
    const regex = /^Gửi\s+tin\s+nhắn\s+(?:'([^']+)'|"([^"]+)"|“([^”]+)”)\s+cho\s+nhóm\s+(.+?)(?:\s+với\s+các\s+nút\s*:\s*(.+))?$/i;
    ...
    if (match[5]) {
        ...
        const options = optionText.split(/\s*,\s*/).map((text, idx) => ({
            index: idx + 1,
            text: text.trim()
    ...
    const result = {
        messageText,
        groupCode,
        messageStart: messageIndex >= 0 ? messageIndex : null,
        messageEnd: messageIndex >= 0 ? messageIndex + messageText.length : null
    };
    if (surveyOptions) {
        result.surveyOptions = surveyOptions;
        result.surveyQuestion = 'Khảo sát';
    }
    return result;
}
...
// Lịch sử broadcast từ database (không dùng file log)
const fileLogBroadcastHistory = {
  set: async (value) => {
    try {
      const parts = value.split(' || ');
      if (parts.length < 14) {
        console.warn('BroadcastHistory.set: không đủ trường', parts.length);
        return Promise.resolve();
      }
      
      await models.BroadcastHistory.create({
        timestamp: parts[0],
        senderId: parts[1],
        groupCode: parts[2],
        messageHtml: parts[3],
        mediaType: parts[4],
        totalSent: parseInt(parts[5]) || 0,
        totalFailed: parseInt(parts[6]) || 0,
        broadcastId: parts[7],
        chainId: parts[8],
        parentBroadcastId: parts[9],
        broadcastTitle: parts[10],
        messageType: parts[11],
        isSurvey: parts[12] === 'true',
        surveyOptions: parts[13],
      });
    } catch (err) {
      console.error('Lỗi lưu BroadcastHistory:', err.message);
    }
    return Promise.resolve();
  },
  ...
  findAll: async () => {
    try {
      const records = await models.BroadcastHistory.findAll({
        order: [['timestamp', 'DESC']],
        raw: true,
      });
      return records || [];
    } catch (err) {
      console.error('Lỗi đọc BroadcastHistory:', err.message);
      return [];
    }
  }
};
// Chi tiết tin nhắn broadcast
// recordAttribute: broadcastId || teleChatId || messageId || timestamp
const fileLogBroadcastMessages = new HActiveRecordFile({
    fileName: 'broadcast_messages',
    recordAttribute: ['broadcastId', 'teleChatId', 'messageId', 'timestamp'],
    expired: 86400 * 30 * 12 * 2,
    limit: 100000
})
// Xác nhận đọc tin nhắn broadcast
// recordAttribute: broadcastId || teleChatId || confirmed || timestamp
const fileLogConfirmations = new HActiveRecordFile({
    fileName: 'broadcast_confirmations',
    recordAttribute: ['broadcastId', 'teleChatId', 'confirmed', 'timestamp'],
    expired: 86400 * 30 * 12 * 2,
    limit: 100000
})
...
const fileLogSurveyAnswers = new HActiveRecordFile({
    fileName: 'broadcast_survey_answers',
    recordAttribute: ['broadcastId', 'teleChatId', 'optionIndex', 'optionText', 'timestamp'],
    expired: 86400 * 30 * 12 * 2,
    limit: 100000
})

// Log debug lỗi bot không gửi tin nhắn vào topic cho GV
const fileLogDebug = new HActiveRecordFile({
    fileName: 'debug_error',
    recordAttribute: ['checkpointChain', 'error', 'timestamp'],
    expired: 86400 * 30 * 12 * 2,
    limit: 100000
})

function getDebugErrorMessage(error, prefix = '') {
    const errorMessage = error && error.message ? error.message : String(error || '');
    return `${prefix ? (prefix + ': ') : ''}${String(errorMessage || '').replace(/\r?\n/g, ' ').replace(/\|\|/g, '|')}`;
}

async function writeDebugLog(checkpointChain, error = '') {
    try {
        await fileLogDebug.set(`${checkpointChain} || ${String(error || '').replace(/\r?\n/g, ' ').replace(/\|\|/g, '|')} || ${new Date().toISOString()}`)
    } catch (e) {
        console.log('writeDebugLog error', e)
    }
}

function formatBroadcastLabel(text = '') {
    const raw = String(text || '').trim();
    if (!raw) return '(No title)';
    return raw.length > 40 ? raw.slice(0, 40) + '...' : raw;
}

function uniqueTeleChatIds(records = []) {
    return Array.from(new Set((records || []).map(r => String(r.teleChatId))));
}

function getBroadcastChainIds(historyRecords = [], chainId) {
    const chainEntries = (historyRecords || []).filter(r => r.chainId === chainId);
    return Array.from(new Set(chainEntries.map(r => r.broadcastId)));
}

function getLatestSurveyAnswers(answers = []) {
    const latest = {};
    for (const answer of answers) {
        const key = String(answer.teleChatId);
        if (!latest[key] || new Date(answer.timestamp) > new Date(latest[key].timestamp)) {
            latest[key] = answer;
        }
    }
...
function countSurveyOptions(answers = [], surveyOptions = []) {
    const latestAnswers = getLatestSurveyAnswers(answers);
    const counts = surveyOptions.map(option => ({ optionText: option.text, optionIndex: option.index, count: 0 }));
    for (const answer of latestAnswers) {
        const match = counts.find(item => item.optionIndex === Number(answer.optionIndex));
        if (match) match.count += 1;
    }
    return counts;
}

async function showBroadcastList(bot, chatId, page = 0) {
    const pageSize = 5;
    const broadcasts = (await fileLogBroadcastHistory.findAll() || [])
        .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    const totalPages = Math.ceil(broadcasts.length / pageSize);
    const start = page * pageSize;
    const end = start + pageSize;
    const pageBroadcasts = broadcasts.slice(start, end);

    if (pageBroadcasts.length === 0) {
        await bot.sendMessage(chatId, 'Không có broadcast nào.');
        return;
    }

    const keyboard = pageBroadcasts.map(b => {
        const title = formatBroadcastLabel(b.broadcastTitle || b.messageHtml || b.groupCode || 'Broadcast');
        const typeLabel = b.messageType === 'reminder' ? 'Nhắc nhở' : 'Thông báo';
        return [{ text: `${typeLabel}: ${title}`, callback_data: `select_broadcast_${b.broadcastId}` }];
    });

    // Thêm nút phân trang
    const paginationButtons = [];
    if (page > 0) {
        paginationButtons.push({ text: '<<< Trang trước', callback_data: `broadcast_page_${page - 1}` });
    }
    if (page < totalPages - 1) {
        paginationButtons.push({ text: 'Trang sau >>>', callback_data: `broadcast_page_${page + 1}` });
    }
    if (paginationButtons.length > 0) {
        keyboard.push(paginationButtons);
    }

    await bot.sendMessage(chatId, `Chọn broadcast để xem báo cáo (Trang ${page + 1}/${totalPages}):`, {
...
async function executeBroadcast(msg, photoFileIds) {
    ...
    const groupMembers = await models.UserGroup.findAll({ where: { groupCode, deletedAt: null }, raw: true });
    const teacherCodes = (groupMembers || []).map(m => {
        return {
            teleChatId: m.teleChatId,
            teacherCode: normalizeTeacherCode(m.userName),
            userName: m.userName
        }
    });
    // Gửi log cho admin Hà
    const shortText = commandText.slice(0, 20) + (commandText.length > 20 ? '...' : '');
    bot.sendMessage("151333233", `Người dùng telegramId: ${senderChatId} yêu cầu gửi tin nhắn cho nhóm ${groupCode} với nội dung: ${shortText}`);
    ...
    // Tạo broadcastId duy nhất
    const broadcastId = `${Date.now()}_${senderId}`;
    const chainId = broadcastId;
    const parentBroadcastId = '';
    ...
    const broadcastTitle = broadcastCommand.messageText || rawText || commandText || 'Broadcast';
    let messageHtml = '';
    ...
    let surveyData = null;
    if (broadcastCommand.surveyOptions) {
        surveyData = {
            options: broadcastCommand.surveyOptions,
            rawOptions: JSON.stringify(broadcastCommand.surveyOptions)
        };
    }
    const isSurveyBroadcast = !!surveyData;
    const surveyOptions = surveyData ? surveyData.options : [];
    const sendOption = {
        parse_mode: 'HTML',
        reply_markup: {
            inline_keyboard: isSurveyBroadcast ? surveyOptions.map(option => [{ text: option.text, callback_data: `survey_${broadcastId}_${option.index}` }]) : [[{ text: "Xác nhận", callback_data: `confirm_${broadcastId}` }]]
        }
    };
    const groupSupportVh = '-1003207607839';
    ...
    const textToSend = messageHtml;
    const ts = new Date().toISOString();
    for (let i = 0; i < teacherCodes.length; i++) {
        const teacherGroup = teacherCodes[i];
        const teacherCode = teacherGroup.teacherCode;
        const userProfile = dataInit.userByUserName[teacherCode] || teacherGroup;
        ...
        try {
            let sentMessage;
            if (isAlbum) {
                const mediaArray = photoFileIds.map(fid => ({
                    type: 'photo',
                    media: fid,
                }));
                await bot.sendMediaGroup(userProfile.teleChatId, mediaArray);
                sentMessage = await bot.sendMessage(userProfile.teleChatId, textToSend || 'Vui lòng xác nhận.', sendOption);
            } else if (isSinglePhoto) {
                sentMessage = await bot.sendPhoto(userProfile.teleChatId, photoFileIds[0], { ...sendOption, caption: textToSend });
            } else if (msg.document) {
                sentMessage = await bot.sendDocument(userProfile.teleChatId, msg.document.file_id, { ...sendOption, caption: textToSend });
            } else if (msg.video) {
                sentMessage = await bot.sendVideo(userProfile.teleChatId, msg.video.file_id, { ...sendOption, caption: textToSend });
            } else {
                sentMessage = await bot.sendMessage(userProfile.teleChatId, textToSend, sendOption);
            }
            // Lưu messageId
            await fileLogBroadcastMessages.set(`${broadcastId} || ${userProfile.teleChatId} || ${sentMessage.message_id} || ${ts}`);
            result.success.push({ teacherCode, teleChatId: userProfile.teleChatId, userName: userProfile.userName, messageId: sentMessage.message_id });
            // Gửi vào thread cho VH
            const threadId = dataInit.threadCrmError[userProfile.teleChatId];
            const threadMessage = `${messageHtml}\n(==> CM thông báo cho GV)`;
            if (threadId) {
                const threadOption = { parse_mode: 'HTML', message_thread_id: threadId };
                try {
                    if (isAlbum) {
                        const threadMediaArray = photoFileIds.map((fid, idx) => ({
                            type: 'photo',
                            media: fid,
                            ...(idx === 0 && threadMessage ? { caption: threadMessage, parse_mode: 'HTML' } : {}),
                        }));
                        await bot.sendMediaGroup(groupSupportVh, threadMediaArray, threadOption);
                    } else if (isSinglePhoto) {
                        await bot.sendPhoto(groupSupportVh, photoFileIds[0], { ...threadOption, caption: threadMessage });
                    } else if (msg.document) {
                        await bot.sendDocument(groupSupportVh, msg.document.file_id, { ...threadOption, caption: threadMessage });
                    } else if (msg.video) {
                        await bot.sendVideo(groupSupportVh, msg.video.file_id, { ...threadOption, caption: threadMessage });
                    } else {
                        await bot.sendMessage(groupSupportVh, threadMessage, threadOption);
                    }
                } catch (threadError) {
                    console.error('Lỗi gửi thông báo vào thread support:', threadError?.Error, userProfile.teleChatId, threadId);
                }
            } else {
                await bot.sendMessage('151333233', `Không tìm thấy topic thread cho giáo viên <b>${escapeHtml(userProfile.userName || teacherCode)}</b> (${userProfile.teleChatId}).\nNội dung: ${escapeHtml(threadMessage)}`, { parse_mode: 'HTML' });
            }
        } catch (error) {
    ...
    try {
        ...
        await fileLogBroadcastHistory.set(
            `${ts} || ${senderId} || ${groupCode} || ${messageHtml.replace(/\|\|/g, '|')} || ${mediaType} || ${result.success.length} || ${result.failed.length} || ${broadcastId} || ${chainId} || ${parentBroadcastId} || ${broadcastTitle.replace(/\|\|/g, '|')} || ${messageType} || ${isSurveyBroadcast} || ${surveyData ? JSON.stringify(surveyData.options).replace(/\|\|/g, '|') : ''}`
        );
    } catch (historyError) {
    ...
    // Ack lại cho admin
    const ackLines = [
        `<b>Broadcast</b> nhóm <b>${escapeHtml(groupCode)}</b> (ID: <code>${broadcastId}</code>)`,
...
export async function receiveMsg(bot){
    ...
    bot.on("callback_query", async (callbackQuery) => {
        ...
        if (data === "dktele") {
            ...
            const rawData = data.slice('survey_'.length);
            const lastSep = rawData.lastIndexOf('_');
            if (lastSep === -1) {
                await bot.answerCallbackQuery(callbackQuery.id, { text: 'Dữ liệu khảo sát không hợp lệ.' });
                return;
            }
            const broadcastId = rawData.slice(0, lastSep);
            const optionIndex = Number(rawData.slice(lastSep + 1));
            const teleChatId = callbackQuery.from.id.toString();
            const ts = new Date().toISOString();
            const historyRecords = await fileLogBroadcastHistory.findAll();
            const selectedHistory = historyRecords.find(r => r.broadcastId === broadcastId);
            let optionText = 'Lựa chọn';
            if (selectedHistory && selectedHistory.surveyOptions) {
                try {
                    const options = JSON.parse(selectedHistory.surveyOptions);
                    const option = options.find(opt => Number(opt.index) === optionIndex);
                    if (option) optionText = option.text;
                } catch (parseError) {
                    console.log('Không parse được surveyOptions:', parseError.message);
                }
            }
            await fileLogSurveyAnswers.set(`${broadcastId} || ${teleChatId} || ${optionIndex} || ${optionText.replace(/\|\|/g, '|')} || ${ts}`);
            await fileLogConfirmations.set(`${broadcastId} || ${teleChatId} || true || ${ts}`);
            await bot.answerCallbackQuery(callbackQuery.id, { text: `Đã ghi nhận: ${optionText}` });
            try {
                ...
                console.log('Không thể edit markup:', editError.message);
            }
        }
        else if (data.startsWith('confirm_')) {
            const broadcastId = data.slice('confirm_'.length);
            const teleChatId = callbackQuery.from.id.toString();
            const ts = new Date().toISOString();
            // Kiểm tra nếu đã confirm chưa
            const allConfirmations = await fileLogConfirmations.findAll();
            const existing = allConfirmations.find(r => r.broadcastId === broadcastId && r.teleChatId === teleChatId);
            if (!existing) {
                await fileLogConfirmations.set(`${broadcastId} || ${teleChatId} || true || ${ts}`);
                await bot.answerCallbackQuery(callbackQuery.id, { text: 'Đã xác nhận!' });
                // Disable nút
                try {
                    await bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: callbackQuery.message.chat.id, message_id: callbackQuery.message.message_id });
                } catch (editError) {
                ...
                await bot.answerCallbackQuery(callbackQuery.id, { text: 'Đã xác nhận trước đó.' });
            }
        }
        else if (data.startsWith('select_broadcast_')) {
            const broadcastId = data.slice('select_broadcast_'.length);
            const historyRecords = await fileLogBroadcastHistory.findAll();
            const selectedHistory = historyRecords.find(r => r.broadcastId === broadcastId);
            if (!selectedHistory) {
                await bot.sendMessage(callbackQuery.message.chat.id, `Không tìm thấy broadcast ID ${broadcastId}.`);
                return;
            }
            const isOriginal = !selectedHistory.messageType || selectedHistory.messageType === 'original';
            const chainId = selectedHistory.chainId || broadcastId;
            const chainIds = getBroadcastChainIds(historyRecords, chainId);
            const allMessages = await fileLogBroadcastMessages.findAll();
            const selectedMessages = allMessages.filter(m => m.broadcastId === broadcastId);
            const chainMessages = allMessages.filter(m => chainIds.includes(m.broadcastId));
            const allConfirmations = await fileLogConfirmations.findAll();
            const confirmedChainIds = new Set(allConfirmations.filter(c => chainIds.includes(c.broadcastId) && c.confirmed === 'true').map(c => String(c.teleChatId)));
            const confirmedSelectedIds = new Set(allConfirmations.filter(c => c.broadcastId === broadcastId && c.confirmed === 'true').map(c => String(c.teleChatId)));
            const totalSelected = uniqueTeleChatIds(selectedMessages).length;
            const totalChain = uniqueTeleChatIds(chainMessages).length;
            const confirmedSelected = Array.from(confirmedSelectedIds).length;
            const confirmedChain = Array.from(confirmedChainIds).length;
            const notConfirmedSelected = totalSelected - confirmedSelected;
            const notConfirmedChain = totalChain - confirmedChain;
            const titleLabel = formatBroadcastLabel(selectedHistory.broadcastTitle || selectedHistory.messageHtml || 'Broadcast');
            let reportText = `<b>${titleLabel}</b>\n`;
            if (isOriginal) {
                reportText += `- Loại: Thông báo gốc\n- Tổng chuỗi: ${totalChain}\n- Đã xác nhận: ${confirmedChain}\n- Chưa xác nhận: ${notConfirmedChain}`;
            } else {
                reportText += `- Loại: Nhắc nhở\n- Tổng gửi: ${totalSelected}\n- Đã xác nhận: ${confirmedSelected}\n- Chưa xác nhận: ${notConfirmedSelected}`;
            }
            if (selectedHistory.isSurvey && selectedHistory.surveyOptions) {
                let surveyOptions = [];
                try {
                    surveyOptions = JSON.parse(selectedHistory.surveyOptions);
                } catch (parseError) {
                    console.log('Không parse được surveyOptions trong báo cáo:', parseError.message);
                }
                if (surveyOptions.length) {
                    const allSurveyAnswers = await fileLogSurveyAnswers.findAll();
                    const surveyAnswers = isOriginal ? allSurveyAnswers.filter(a => chainIds.includes(a.broadcastId)) : allSurveyAnswers.filter(a => a.broadcastId === broadcastId);
                    const counts = countSurveyOptions(surveyAnswers, surveyOptions);
                    reportText += `\n\n<b>Thống kê khảo sát</b>`;
            ...
            const hasUnconfirmed = isOriginal ? notConfirmedChain > 0 : notConfirmedSelected > 0;
            if (hasUnconfirmed) {
                inlineKeyboard.push([{ text: 'Gửi nhắc nhở', callback_data: `send_reminder_${broadcastId}` }]);
            }
            await bot.sendMessage(callbackQuery.message.chat.id, reportText, {
                parse_mode: 'HTML',
                reply_markup: inlineKeyboard.length > 0 ? { inline_keyboard: inlineKeyboard } : undefined
            });
        }
        else if (data.startsWith('send_reminder_')) {
            const broadcastId = data.slice('send_reminder_'.length);
            queueChatPrivate[callbackQuery.message.chat.id] = `waiting_reminder_${broadcastId}`;
            await bot.sendMessage(callbackQuery.message.chat.id, 'Gửi tin nhắn nhắc nhở (text, ảnh, video):');
        }
        else if (data.startsWith('broadcast_page_')) {
            const page = parseInt(data.slice('broadcast_page_'.length), 10) || 0;
            await showBroadcastList(bot, callbackQuery.message.chat.id, page);
        }
        //bot.sendMessage(callbackQuery.message.chat.id, "You clicked button " + data);
    });

    bot.on("message", async (msg) => {
        let checkPoint = [msg.text ? msg.text.slice(0, 100) : "Other message", 1];
        try {
            ...
            // Xử lý /send-msg-module
            if (msg.text === '/send-msg-module' && allowedBroadcastSenderIds.has(msg.from.id.toString())) {
                checkPoint.push(2);
                await showBroadcastList(bot, msg.chat.id, 0);
                return;
            }

            // Xử lý tin nhắn reminder
            if (queueChatPrivate[msg.chat.id] && queueChatPrivate[msg.chat.id].startsWith('waiting_reminder_')) {
                checkPoint.push(3);
                const broadcastId = queueChatPrivate[msg.chat.id].slice('waiting_reminder_'.length);
                delete queueChatPrivate[msg.chat.id];

                const historyRecords = await fileLogBroadcastHistory.findAll();
                const selectedHistory = historyRecords.find(r => r.broadcastId === broadcastId);
                if (!selectedHistory) {
                    await bot.sendMessage(msg.chat.id, `Không tìm thấy broadcast ID ${broadcastId}.`);
                    return;
                }
                const isOriginal = !selectedHistory.messageType || selectedHistory.messageType === 'original';
                const chainId = selectedHistory.chainId || broadcastId;
                ...
                const allConfirmations = await fileLogConfirmations.findAll();
                const confirmedChainIds = new Set(allConfirmations.filter(c => chainIds.includes(c.broadcastId) && c.confirmed === 'true').map(c => String(c.teleChatId)));
                const selectedRecipientIds = uniqueTeleChatIds(selectedMessages);
                const chainRecipientIds = uniqueTeleChatIds(chainMessages);
                let toRemindIds = [];
                if (isOriginal) {
                    toRemindIds = chainRecipientIds.filter(id => !confirmedChainIds.has(id));
                } else {
                    toRemindIds = selectedRecipientIds.filter(id => !confirmedChainIds.has(id));
                }

                const reminderBroadcastId = `${Date.now()}_${msg.from.id}`;
                const reminderChainId = chainId;
                const parentBroadcastId = broadcastId;
                const messageType = 'reminder';
                const reminderTitle = msg.caption || msg.text || 'Nhắc nhở';
                const reminderType = msg.photo ? 'photo' : msg.video ? 'video' : msg.document ? 'document' : 'text';

                let reminderSurveyOptions = [];
                if (selectedHistory.isSurvey && selectedHistory.surveyOptions) {
                    try {
                        reminderSurveyOptions = JSON.parse(selectedHistory.surveyOptions);
                    } catch (parseError) {
                        console.log('Không parse được surveyOptions cho reminder:', parseError.message);
                    }
                }
                ...
                const reminderOption = {
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: reminderSurveyOptions.length
                            ? reminderSurveyOptions.map(option => [{ text: option.text, callback_data: `survey_${reminderBroadcastId}_${option.index}` }])
                            : [[{ text: "Xác nhận", callback_data: `confirm_${reminderBroadcastId}` }]]
                    }
                };
                ...
                const ts = new Date().toISOString();
                for (const teleChatId of toRemindIds) {
                    try {
                        if (msg.photo && msg.photo.length) {
                            const photoId = msg.photo[msg.photo.length - 1].file_id;
                            const sent = await bot.sendPhoto(teleChatId, photoId, { ...reminderOption, caption: msg.caption || '' });
                            await fileLogBroadcastMessages.set(`${reminderBroadcastId} || ${teleChatId} || ${sent.message_id} || ${ts}`);
                        } else if (msg.video) {
                            const sent = await bot.sendVideo(teleChatId, msg.video.file_id, { ...reminderOption, caption: msg.caption || '' });
                            await fileLogBroadcastMessages.set(`${reminderBroadcastId} || ${teleChatId} || ${sent.message_id} || ${ts}`);
                        } else if (msg.document) {
                            const sent = await bot.sendDocument(teleChatId, msg.document.file_id, { ...reminderOption, caption: msg.caption || '' });
                            await fileLogBroadcastMessages.set(`${reminderBroadcastId} || ${teleChatId} || ${sent.message_id} || ${ts}`);
                        } else if (msg.text) {
                            const sent = await bot.sendMessage(teleChatId, msg.text, reminderOption);
                            await fileLogBroadcastMessages.set(`${reminderBroadcastId} || ${teleChatId} || ${sent.message_id} || ${ts}`);
                        }
                        reminderSent++;
                        await new Promise(r => setTimeout(r, 300));
                    } catch (error) {
                        console.log('Lỗi gửi reminder:', error);
                        reminderFailed++;
                    }
                }
                try {
                    await fileLogBroadcastHistory.set(
                        `${ts} || ${msg.from.id} || ${selectedHistory.groupCode || ''} || ${reminderTitle.replace(/\|\|/g, '|')} || ${reminderType} || ${reminderSent} || ${reminderFailed} || ${reminderBroadcastId} || ${reminderChainId} || ${parentBroadcastId} || ${reminderTitle.replace(/\|\|/g, '|')} || ${messageType} || ${selectedHistory.isSurvey ? 'true' : 'false'} || ${selectedHistory.surveyOptions ? selectedHistory.surveyOptions.replace(/\|\|/g, '|') : ''}`
                    );
                } catch (error) {
                    console.error('Lỗi lưu lịch sử reminder:', error);
                }
                await bot.sendMessage(msg.chat.id, `Đã gửi nhắc nhở cho ${reminderSent} người. (${reminderFailed} lỗi)`);
                return;
            }
            ...
            try {
                ...
                const commandText = (msg.caption ? msg.caption : (msg.text ? msg.text : '')).trim();
                const broadcastCommand = parseBroadcastCommand(commandText);
                console.log("broadcastCommand: ", broadcastCommand)
                console.log("isAllowedSender: ", isAllowedSender)
                ...
                await writeDebugLog(checkPoint.join("->"), getDebugErrorMessage(broadcastError, 'broadcastError'))
            }
            ...
            if(rPhoto && !rMediaGroupId){
                ...
                */
                ...
                await writeDebugLog(checkPoint.join("->"), 'success');
            ...
            await writeDebugLog(checkPoint.join("->"), getDebugErrorMessage(error, 'receiveMsg err'));
        }
    });
        ...
        await writeDebugLog("receiveMsg err", getDebugErrorMessage(err, 'receiveMsg err'));
    }
}
...
export async function sendBulkMessageToTeachers(bot, message, teacherCodes, options = {}) {
    ...
    for (let i = 0; i < teacherCodes.length; i++) {
        ...
        if (!userProfile || !userProfile.teleChatId) {
            result.failed.push({
                teacherCode: teacherCode,
                reason: 'Không tìm thấy giáo viên hoặc giáo viên chưa chat với bot'
            });
            continue;
        }
        try {
